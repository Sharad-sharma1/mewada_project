...
class OderFilter(django_filters.FilterSet):
    ...
    name = django_filters.CharFilter(label_suffix='', label='Name',widget=forms.TextInput(attrs={'class':'form-control form-control-sm '}),lookup_expr='icontains')
...
class OderFilter2(OderFilter):
    ...
    receipt_date = django_filters.DateFilter(label_suffix='',label='Receipt Date', widget=DateInput(attrs={'class':'form-control form-control-sm '}),lookup_expr='icontains')

    class Meta:
        model = Premium_User_Table
        fields = ['name','father_or_husband_name','area_id','village_id', 'mobile_no1', 'mobile_no2', 'death_flag', 'receipt_no', 'receipt_date']
